...
int fifo(int8_t** page_table, int num_pages, int prev_page,
        ...
        // fifo_frm é o frame de inicio da fila, então é feito um loop 
        // buscando na tabela a pagina com esse valor e assim retornando 
        // como pagina a ser substituida
    for (int i = 0; i < num_pages; i++) {
        if (page_table[i][PT_FRAMEID] == fifo_frm) return i;
    }
}

int second_chance(int8_t** page_table, int num_pages, int prev_page,
                  int fifo_frm, int num_frames, int clock) {
    // primeiro loop onde j percorre o numero de molduras pois a fila 
    // deve ser alterada caso exista uma segunda chance no primeiro da fila
	for (int j = fifo_frm; 1; j = (j + 1) % num_frames) {
        // i percorre as paginas para procurar quem pertence ao frame j
		for (int i = 0; i < num_pages; i++) {
			if (page_table[i][PT_FRAMEID] == j) {
				if (page_table[i][PT_REFERENCE_BIT] == 0) {
                    // caso o bit de referencia é 0 ele n tem segunda 
                    // chance, então é retornado como elemento a ser 
                    // substituido do frame.
					return i;
				} else {
                    // se o bit de referencia é 1 ele é decrementado e o 
                    // loop passa para o proximo elemento da fila 
                    // utilizando (j + 1) % num_frame
